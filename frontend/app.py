...
def init_sqlite_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS verified_entities
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS unverified_entities
                 (entity_type TEXT, entity_value TEXT, PRIMARY KEY (entity_type, entity_value))''')
...
def verify_entity(entity_type, entity_value):
    ...
    c_verified.execute("INSERT INTO verified_entities VALUES (?, ?)", (entity_type, entity_value))
    conn_verified.commit()
    conn_verified.close()
    
    conn_ontology = sqlite3.connect('ontology.db')
    c_ontology = conn_ontology.cursor()
    c_ontology.execute("DELETE FROM unverified_entities WHERE entity_type=? AND entity_value=?", 
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Select Page",
        ["Neo4j Query", "Trend Analysis", "Verification", "Ontology"]
    )
