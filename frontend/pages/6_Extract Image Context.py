...
def call_extraction_engine_api():
    ...
    payload = {"csv_path_in": ingested_csv_path, "db_path_out": extracted_db_path}
...
def image_context_extraction_page():
    ...
    for i in range(num_images):
        ...
        with col2:
            image_link = st.text_input(
                label="", placeholder="Enter Image link", key=f"image_link_{i}"
            )
            image_links.append(image_link)
        with col3:
            image_text = st.text_input(
                label="",
                placeholder=f"Enter Image Text Context {i + 1} (if available):",
            )
    ...
    if st.button("Extract", key="extract_button", help="Click to start extraction"):
        with st.spinner("Processing..."):
            # call_extraction_engine_api()
            time.sleep(30)  # Simulate processing time
        df = pd.read_csv(ingested_csv_path)
        st.dataframe(df)
